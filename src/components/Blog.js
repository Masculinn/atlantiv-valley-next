...
const BlogDiv = ({title, des, imgAlt, imgSource, sector, linkedinUrl}) => {
    return (
      <div className='w-full max-w-md h-full my-16 shadow-2xl mx-4 lg:mx-12 lg:w-1/4 text-black cursor-pointer' id='section_font'>
        <a href={linkedinUrl} rel="noreferrer" target='_blank' className='hover:underline hover:text-blue-700'>
          <div className='w-full text-start h-1/3 px-8 justify-center tracking-tight text-'>
        ...
        <Image onClick={() => {swal({icon: imgSource})}} src={imgSource} className='h-auto w-full mx-auto transform transition-all duration-500 hover:scale-105 hover:translate-x-5 hover:translate-y-5 hover:shadow-2xl' alt={imgAlt} />
      </div>
  )
  }
  const Blog = (props) => {
